  ...
  <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-50/20 p-4 md:p-6 space-y-5">
   ...
   <Tabs defaultValue="products" className="space-y-4">
    <div className="bg-white rounded-xl p-1.5 border border-slate-100 shadow-sm overflow-x-auto">
     <TabsList className="bg-transparent inline-flex w-auto min-w-full gap-1">
      ...
      <TabsTrigger 
       ...
       className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-rose-500 data-[state=active]:to-rose-600 data-[state=active]:text-white data-[state=active]:shadow-sm data-[state=inactive]:text-slate-500 data-[state=inactive]:bg-transparent rounded-lg px-3 py-2 text-sm font-medium transition-all"
    ...
    <TabsContent value="stock-out-history" className="space-y-4">
     <Card className="rounded-xl border border-slate-100 bg-white shadow-sm overflow-hidden">
      <div className="p-4 border-b border-slate-100 bg-gradient-to-r from-rose-50/80 to-transparent">
       <div className="flex flex-row items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
         ...
         <div>
          <h2 className="text-base font-semibold text-slate-800">Stock Out History</h2>
          <p className="text-xs text-slate-500">
           <span className="font-semibold text-rose-600 tabular-nums">{filteredStockOutHistory.length}</span> items sold through POS
          </p>
         </div>
        </div>
        <div className="flex gap-2 flex-wrap">
      ...
      <CardContent className="p-5">
       {stockOutLoading ? (
        ...
        <div className="text-center py-12">
         <div className="p-4 bg-slate-100 rounded-2xl inline-block mb-4">
          <ArrowUpCircle className="h-12 w-12 text-slate-400" />
         </div>
         <h3 className="text-lg font-semibold text-slate-900 mb-2">No stock out records found</h3>
         <p className="text-slate-500 mb-4">Items sold through POS will appear here</p>
         <Button 
          onClick={() => refetchStockOutHistory()}
          className="bg-gradient-to-r from-rose-500 to-rose-600 text-white shadow-md"
         >
          <RefreshCw className="h-4 w-4 mr-2" />
          Refresh History
         </Button>
        ...
        <div className="space-y-4">
         ...
         <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {visibleStockOutHistory.map((item) => (
           <Card 
            key={item.id}
            className="rounded-xl p-4 border border-slate-200 hover:shadow-lg transition-all"
           >
            <CardContent className="p-0">
             <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
               <div className="flex items-center gap-2">
                ...
                <div>
                 <p className="font-semibold text-slate-900">{item.color?.colorName || 'Unknown'}</p>
                 <p className="text-xs text-slate-400">{item.color?.colorCode || '-'}</p>
                </div>
               </div>
              ...
              <div className="space-y-2">
               ...
               <div className="flex justify-between items-center text-sm">
                <span className="text-slate-500">Product</span>
                <span className="text-slate-400 truncate">{item.color?.variant?.product?.productName || '-'}</span>
               </div>
               <div className="flex justify-between items-center text-sm">
                <span className="text-slate-500">Size</span>
                <Badge variant="outline" className="bg-slate-100 text-slate-600 border-slate-200">
