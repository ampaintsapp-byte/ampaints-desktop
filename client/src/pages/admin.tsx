...
type AdminSection = "overview" | "operations" | "store" | "security" | "data"
type OperationsTab = "stock" | "sales" | "payments" | "returns" | "unpaid"
...
const sectionConfig = [
  ...
  { id: "operations" as const, label: "Operations", icon: ClipboardList, description: "Audit & Reports" },
...
export default function Admin() {
  ...
  const [operationsTab, setOperationsTab] = useState<OperationsTab>("stock")
            ...
            {activeSection === "operations" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground">Audit reports and transaction history</p>
                </div>

                <Tabs value={operationsTab} onValueChange={(v) => setOperationsTab(v as OperationsTab)}>
                            ...
                            <div className="border rounded-lg overflow-hidden">
                                ...
                                <TableBody>
                                  {colors
                                    .filter(c => 
                                      ...
                                      c.colorName?.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
                                      c.colorCode?.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
                                      c.variant?.name?.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
                                      c.variant?.product?.name?.toLowerCase().includes(debouncedSearchQuery.toLowerCase())
                                    )
                                    .slice(0, visibleLimit)
                                    .map((color) => (
                                      <TableRow key={color.id}>
                                        <TableCell className="font-medium">{color.variant?.product?.name || "-"}</TableCell>
                                        <TableCell>{color.variant?.name || "-"}</TableCell>
                                        <TableCell>{color.colorName}</TableCell>
                                        <TableCell><Badge variant="outline">{color.colorCode}</Badge></TableCell>
                                        <TableCell className="text-right">
                                          <Badge variant={(color.stockQuantity || 0) < 10 ? "destructive" : "secondary"}>
                              ...
                              {colors.length > visibleLimit && (
                                <div className="p-4 text-center border-t">
                                  <Button variant="outline" onClick={() => setVisibleLimit(v => v + VISIBLE_LIMIT_INCREMENT)}>
                                    Load More ({colors.length - visibleLimit} remaining)
                                  </Button>
                                </div>
                              )}
                            </div>
                                  ...
                                  {allSales
                                    .filter(s => {
                                      const matchSearch = !debouncedSearchQuery || 
                                        ...
                                        s.customerPhone?.includes(debouncedSearchQuery)
                                      const saleDate = new Date(s.createdAt)
                                      const matchDateFrom = !dateFrom || saleDate >= startOfDay(new Date(dateFrom))
                                      const matchDateTo = !dateTo || saleDate <= endOfDay(new Date(dateTo))
                                      return matchSearch && matchDateFrom && matchDateTo
                                    })
                                    .slice(0, visibleLimit)
                                    .map((sale) => (
                                      <TableRow key={sale.id}>
                                        <TableCell>{formatDateShort(new Date(sale.createdAt))}</TableCell>
                                        <TableCell className="font-medium">{sale.customerName}</TableCell>
                                        <TableCell>{sale.customerPhone}</TableCell>
                                        <TableCell className="text-right">{formatCurrency(safeParseFloat(sale.totalAmount))}</TableCell>
                                        <TableCell className="text-right text-green-600">{formatCurrency(safeParseFloat(sale.amountPaid))}</TableCell>
                                        <TableCell>
                                          <Badge variant={sale.paymentStatus === "paid" ? "secondary" : "destructive"}>
                                            {sale.paymentStatus}
                                          </Badge>
                                        </TableCell>
                                      </TableRow>
                            ...
                            <Table>
                              ...
                              <TableBody>
                                {paymentHistory
                                  .filter(p => {
                                    const matchSearch = !debouncedSearchQuery || 
                                      p.sale?.customerName?.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
                                      p.sale?.customerPhone?.includes(debouncedSearchQuery)
                                    const payDate = new Date(p.paymentDate)
                                    const matchDateFrom = !dateFrom || payDate >= startOfDay(new Date(dateFrom))
                                    const matchDateTo = !dateTo || payDate <= endOfDay(new Date(dateTo))
                                    return matchSearch && matchDateFrom && matchDateTo
                                  })
                                  .slice(0, visibleLimit)
                                  .map((payment) => (
                                    <TableRow key={payment.id}>
                                      <TableCell>{formatDateShort(new Date(payment.paymentDate))}</TableCell>
                                      <TableCell className="font-medium">{payment.sale?.customerName || "-"}</TableCell>
                                      <TableCell>
                                        <Badge variant="secondary">{payment.paymentType || "payment"}</Badge>
                                      </TableCell>
                              ...
                              <TableBody>
                                {returns
                                  .filter(r => {
                                    ...
                                    const matchDateFrom = !dateFrom || returnDate >= startOfDay(new Date(dateFrom))
                                    const matchDateTo = !dateTo || returnDate <= endOfDay(new Date(dateTo))
                                    return matchSearch && matchDateFrom && matchDateTo
                                  })
                                  .slice(0, visibleLimit)
                                  .map((ret) => (
                                    <TableRow key={ret.id}>
                                      <TableCell>{formatDateShort(new Date(ret.returnDate))}</TableCell>
                                      <TableCell>
                                      ...
                                      <TableCell className="text-right text-red-600">
                                        {formatCurrency(safeParseFloat(ret.refundAmount))}
                                      </TableCell>
                              ...
                              <TableBody>
                                {(() => {
                                  const unpaidSales = allSales.filter(s => 
                                    ...
                                    s.paymentStatus !== "full_return" &&
                                    safeParseFloat(s.totalAmount) > safeParseFloat(s.amountPaid)
                                  )
                                  const grouped = unpaidSales.reduce((acc, sale) => {
                                    ...
                                    acc[key].totalAmount += safeParseFloat(sale.totalAmount)
                                    acc[key].amountPaid += safeParseFloat(sale.amountPaid)
                                    return acc
                                  }, {} as Record<string, { customerName: string; customerPhone: string; totalAmount: number; amountPaid: number }>)
                                  ...
                                  return customers.length > 0 ? customers.map((customer, idx) => (
                                    <TableRow key={idx}>
                                      ...
                                      <TableCell className="text-right text-green-600">{formatCurrency(customer.amountPaid)}</TableCell>
                                      <TableCell className="text-right font-bold text-red-600">
                                        {formatCurrency(customer.totalAmount - customer.amountPaid)}
                                      </TableCell>
                                    </TableRow>
                                  )) : (
                                    <TableRow>
