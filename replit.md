...
- **Audit Reports**: Comprehensive PIN-protected audit section with five report tabs: Stock Audit (IN/OUT/RETURN movements, current inventory), Sales Audit (all sales, payments, outstanding), Unpaid Bills (outstanding balances, due dates), Payment History (all payments with bill context), and Returns (full bill and item returns). Features secure 4-digit PIN verification with SHA-256 salted hash storage, session-based token authentication (24-byte random token, 1-hour TTL), downloadable branded PDF reports for each tab, and comprehensive filtering options. Default PIN is "0000" with change prompt.
